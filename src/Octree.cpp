...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	for (int i = 0; i < boxList.size(); i++) {
		...
		int count = getMeshPointsInBox(mesh, node.points, boxList[i], pointsRtn);
		if (count > 0) {
...
// intersect: recursive function that returns true if the ray intersects a box in the node and false otherwise.
// Returns the node with the fewest points that is intersected by the ray and stores it in nodeRtn.
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	bool intersects = node.box.intersect(ray, 0.0, 10000.0);
	if (node.children.size() == 0 && intersects) {
		nodeRtn = node;
		return true;
	}
	else if (intersects) {
...
// intersect: recursive function that returns true if the node's box overlabs with box and false otherwise.
// stores each of the boxes that overlap with box in boxListRtn.
...
// draw: recursive function that draws each box in the octree.
